...
state_tables<-lapply(sim_results,function(tree)table(tree$tip.state)/
                      sum(tree$tip.state))
state_tables
df<-data.frame(speciation_1=speciation_1range,tip_state_0=rep(0,
              length(speciation_1range)),tip_state_1=rep(0,
              length(speciation_1range)))
...
ggplot(nona_df,aes(x=speciation_1))+
  ...
  scale_color_manual(values=c("Tip State 0"="red","Tip State 1"=
                                "black"))+
  xlab("Speciation Rate for State 1")+
  ylab("Tip State Frequency")+
  ggtitle("Speciation vs Tip State Frequency")+
  theme_bw()+
  labs(color="Tip States")
nona_df
#The tip state frequency decreases as the speciation rate increases going closer to the 1 maximum. 
#The tip state 1 frequency stays at one no matter what the speciation rate is. 